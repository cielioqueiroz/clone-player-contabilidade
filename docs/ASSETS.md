...
The shared footer presents all nine badge files as company-published seals and labels them from the centralized credential catalog. Social destinations are centralized in `src/content/catalog.ts`: Instagram from the established public profile, Facebook confirmed directly by the user as `https://www.facebook.com/playercontabilidade`, LinkedIn as the public company page, and WhatsApp through the existing external-company link builder. The location panel uses the user-supplied Maps coordinates and defers the external iframe until an explicit visitor action.
